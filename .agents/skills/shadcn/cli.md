...
> **IMPORTANT:** Always run commands using the project's package runner: `pnpm dlx shadcn@latest`, `pnpm dlx shadcn@latest`, or `bunx --bun shadcn@latest`. Check `packageManager` from project context to choose the right one. Examples below use `pnpm dlx shadcn@latest` but substitute the correct runner for the project.
...
- **Re-install** → `pnpm dlx shadcn@latest init --preset <code> --force --reinstall`. Overwrites all component files with the new preset styles. Use when the user hasn't customized components.

...
export const getAdminBusinessIntelligence = procedures.auth.query(async ({ ctx }) => {
	...
	const customerIds = Array.from(new Set(orderRows.map((order) => order.customerId)));
	...
	const itemsRows =
		orderIds.length > 0
			...
			: [];
	const productIds = Array.from(new Set(itemsRows.map((item) => item.productId)));
	const productsRows =
		productIds.length > 0
			? await db
					.select({
	...
	const customerNameById = new Map(customersRows.map((customer) => [customer.id, customer.name]));
	...
	const totalRevenue = orderRows.reduce((sum, order) => sum + getOrderTotal(order.id), 0);
	...
	const inProgressOrders = orderRows.filter((order) => order.status === 'in_progress');
	...
	const deliveredOrders = orderRows.filter((order) => order.shippingStatus === 'delivered');
	const overdueUnpaidOrders = orderRows.filter(
		(order) =>
			order.paymentStatus === 'pending' && isOrderLate(order.status, order.expectedDeliveryAt),
	);
	...
	const onTimeDeliveryRate =
		deliveredOrders.length > 0
			? Math.round(
					((deliveredOrders.length - lateOrders.filter((order) => order.shippingStatus === 'delivered').length) /
		...
		orderRows.length > 0
			? Math.round(
					(orderRows.reduce(
						(sum, order) =>
							sum +
							(order.expectedDeliveryAt.getTime() - order.createdAt.getTime()) /
								(1000 * 60 * 60 * 24),
						0,
					) /
						orderRows.length) *
	...
	const currentMonthOrders = orderRows.filter(
		(order) => order.createdAt >= currentMonthStart && order.createdAt <= currentMonthEnd,
	);
	...
	const revenueTrend =
		previousMonthRevenue > 0
			? Math.round(((currentMonthRevenue - previousMonthRevenue) / previousMonthRevenue) * 10000) /
				100
	...
	for (let offset = 5; offset >= 0; offset -= 1) {
		const date = new Date(now.getFullYear(), now.getMonth() - offset, 1, 0, 0, 0, 0);
	...
	const ordersByShippingStatus = SHIPPING_STATUS_VALUES.map((status) => ({
		status,
		count: orderRows.filter((order) => order.shippingStatus === status).length,
	}));
	const paymentSplit = [
		{
			status: 'paid',
	...
	>();
	for (const order of orderRows) {
		const customerName = customerNameById.get(order.customerId) ?? order.customerId;
	...
	const topProducts = Array.from(productUnitsMap.values())
		.map((product) => ({
			...product,
			productName:
				productNameById.get(product.productId) ?? product.productId.slice(0, 8),
		}))
		.sort((a, b) => b.revenue - a.revenue)
		.slice(0, 8);

	const unassignedOrders = orderRows.filter((order) => order.assignedToUserId === null);
	const assignmentCoverage =
		orderRows.length > 0
			? Math.round(((orderRows.length - unassignedOrders.length) / orderRows.length) * 10000) /
				100
			: 0;
	...
	for (const order of orderRows) {
		...
		const name =
			userNameById.get(order.assignedToUserId) ?? order.assignedToUserId.slice(0, 8);
		...
		.map((member) => ({
			...
			completionRate:
				member.assignedOrders > 0
					? Math.round((member.completedOrders / member.assignedOrders) * 10000) / 100
					: 0,
		}))
		.sort((a, b) => b.revenue - a.revenue)
		.slice(0, 8);
	...
	const topDelayedOrders = orderRows
		...
		.map((order) => {
			const delayDays = Math.ceil(
				(now.getTime() - order.expectedDeliveryAt.getTime()) / (1000 * 60 * 60 * 24),
			);
			return {
				orderId: order.id,
				customerName: customerNameById.get(order.customerId) ?? order.customerId,
				assignedTo:
